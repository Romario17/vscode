...
DLinkedListNode *dlinkedListNode_new(void *data)
{
...
void dlinkedList_insertFirst(DLinkedList *dlinked_list, void *data)
{
    DLinkedListNode *node = dlinkedListNode_new(data);
...
void dlinkedList_insertLast(DLinkedList *dlinked_list, void *data)
{
    DLinkedListNode *node = dlinkedListNode_new(data);
    if (dlinked_list->first == NULL)
        dlinked_list->first = dlinked_list->last = node;
...
bool dlinkedList_itrInsertCurrent(DLinkedList *dlinked_list, void *data)
{
...
{
    return (dlinked_list->current == NULL) ? NULL : dlinked_list->current->data;
}
...
void dlinkedList_destroy(DLinkedList *dlinked_list, bool destroy_data)
{
    ...
    while (dlinked_list->first != NULL)
    {
        ...
        dlinked_list->first = dlinked_list->first->next;
        if(destroy_data && (node->data != NULL))
            free(node->data);
        free(node);
    }
    free(dlinked_list);
